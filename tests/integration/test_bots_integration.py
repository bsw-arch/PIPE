...
@pytest.mark.asyncio
async def test_data_processor_bot_initialization(
    bot_config, event_bus, state_manager, metrics_collector
):
...
@pytest.mark.asyncio
async def test_monitor_bot_initialization(
    bot_config, event_bus, state_manager, metrics_collector
):
...
@pytest.mark.asyncio
async def test_bot_communication(
    bot_config, event_bus, state_manager, metrics_collector
):
    ...
    pipeline_config = {
        "id": "test_pipeline_1",
        "name": "Test Pipeline",
        "stages": [{"name": "stage1", "type": "task", "duration": 0.1}],
        "enabled": True,
    }
...
@pytest.mark.asyncio
async def test_data_processor_submit_and_process(
    bot_config, event_bus, state_manager, metrics_collector
):
