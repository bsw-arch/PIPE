...
resource "null_resource" "deploy_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      helm upgrade --install pipe-task-bot \
        ../../../helm/charts/pipe-task-bot \
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      kubectl wait --for=condition=ready pod \
        -l app.kubernetes.io/name=pipe-task-bot \
...
output "deployment_commands" {
  ...
  value = <<-EOT
    ...
    # View logs
    kubectl logs -n ${module.k3s_cluster.namespace} -l app.kubernetes.io/name=pipe-task-bot

    # Port forward
    kubectl port-forward -n ${module.k3s_cluster.namespace} svc/pipe-task-bot 8080:8080

    # Execute shell
    kubectl exec -n ${module.k3s_cluster.namespace} -it deployment/pipe-task-bot-task-bot -- sh
