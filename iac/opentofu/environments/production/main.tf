...
resource "null_resource" "deploy_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      helm upgrade --install pipe-artifact-bot \
        ../../../helm/charts/pipe-artifact-bot \
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      kubectl wait --for=condition=ready pod \
        -l app.kubernetes.io/name=pipe-artifact-bot \
...
output "deployment_commands" {
  ...
  value = <<-EOT
    ...
    # View logs
    kubectl logs -n ${module.k3s_cluster.namespace} -l app.kubernetes.io/name=pipe-artifact-bot

    # Port forward
    kubectl port-forward -n ${module.k3s_cluster.namespace} svc/pipe-artifact-bot 8080:8080

    # Execute shell
    kubectl exec -n ${module.k3s_cluster.namespace} -it deployment/pipe-artifact-bot-task-bot -- sh
